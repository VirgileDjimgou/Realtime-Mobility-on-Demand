...
public class ContactEditorFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		if (Version.sdkStrictlyBelow(Version.API11_HONEYCOMB_30)) {
			lastName.setOnClickListener(new OnClickListener() {
				@Override
				public void onClick(View v) {
					InputMethodManager imm = (InputMethodManager) MainActivity_with_Drawer.instance().getSystemService(Context.INPUT_METHOD_SERVICE);
				...
				public void onClick(View v) {
					...
					delete.setOnClickListener(new OnClickListener() {
						...
						public void onClick(View view) {
							contact.delete();
							// MainActivity_with_Drawer.instance().displayContacts(false);
							dialog.dismiss();
						}
					});
		...
		contactPicture.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View view) {
				pickImage();
				MainActivity_with_Drawer.instance().checkAndRequestCameraPermission();
			}
		});
	...
	private int getThumbnailSize() {
		int value = -1;
		Cursor c = MainActivity_with_Drawer.instance().getContentResolver().query(DisplayPhoto.CONTENT_MAX_DIMENSIONS_URI, new String[] { DisplayPhoto.THUMBNAIL_MAX_DIM }, null, null, null);
		try {
			c.moveToFirst();
			value = c.getInt(0);
		} catch (Exception e) {
