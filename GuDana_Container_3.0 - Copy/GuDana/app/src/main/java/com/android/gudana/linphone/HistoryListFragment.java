...
public class HistoryListFragment extends Fragment implements OnClickListener, OnItemClickListener, ContactsUpdatedListener {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		mInflater = inflater;
		View view = inflater.inflate(R.layout.lin_history, container, false);
	...
	public void displayFirstLog(){
		if (mLogs != null && mLogs.size() > 0) {
			...
			if (log.getDirection() == CallDirection.Incoming) {
				MainActivity_with_Drawer.instance().displayHistoryDetail(mLogs.get(0).getFrom().toString(), mLogs.get(0));
			} else {
				MainActivity_with_Drawer.instance().displayHistoryDetail(mLogs.get(0).getTo().toString(), mLogs.get(0));
			}
		} else {
			MainActivity_with_Drawer.instance().displayEmptyFragment();
		}
	}
	...
	public void onResume() {
		...
		if (MainActivity_with_Drawer.isInstanciated()) {
			// MainActivity_with_Drawer.instance().selectMenu(FragmentsAvailable.HISTORY_LIST);
			// MainActivity_with_Drawer.instance().hideTabBar(false);
			MainActivity_with_Drawer.instance().displayMissedCalls(0);
		}
	...
	public void onClick(View v) {
		...
		if (id == R.id.delete) {
			...
			final Dialog dialog = MainActivity_with_Drawer.instance().displayDialog(getString(R.string.delete_text));
	...
	public void onItemClick(AdapterView<?> adapter, View view, int position, long id) {
		if (isEditMode) {
			...
			if (MainActivity_with_Drawer.isInstanciated()) {
				...
				MainActivity_with_Drawer.instance().setAddresGoToDialerAndCall(address.asStringUriOnly(), address.getDisplayName(), null);
			}
		}
	}
	...
	class CallHistoryAdapter extends BaseAdapter {
		...
		public View getView(final int position, View convertView, ViewGroup parent) {
			...
			if (convertView != null) {
				...
				view = mInflater.inflate(R.layout.lin_history_cell, parent,false);
			...
			if (isEditMode) {
				...
				holder.detail.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						if (MainActivity_with_Drawer.isInstanciated()) {
							MainActivity_with_Drawer.instance().displayHistoryDetail(sipUri, log);
						}
					}
				});
			}
