...
import com.android.gudana.linphone.LinphonePreferences.AccountBuilder;
import com.android.gudana.linphone.assistant.AssistantActivity;
import org.linphone.core.LinphoneAccountCreator;
import org.linphone.core.LinphoneCoreException;
import org.linphone.core.LinphoneCoreFactory;
import org.linphone.mediastream.Log;
import com.android.gudana.linphone.ui.PreferencesListFragment;
...
public class AccountPreferencesFragment extends PreferencesListFragment implements LinphoneAccountCreator.LinphoneAccountCreatorListener {
	...
	public AccountPreferencesFragment() {
		super(R.xml.lin_account_preferences);
	...
	private void initAccountPreferencesFields(PreferenceScreen parent) {
		...
		if (mPrefs.getAccountDomain(n).compareTo(getString(R.string.default_domain)) == 0) {
			...
			changePassword.setOnPreferenceClickListener(new OnPreferenceClickListener() {
				public boolean onPreferenceClick(Preference preference) {
					final AlertDialog.Builder alert = new AlertDialog.Builder(MainActivity_with_Drawer.instance());
					LayoutInflater inflater = MainActivity_with_Drawer.instance().getLayoutInflater();
					View layout = inflater.inflate(R.layout.lin_new_password, null);
					...
					alert.setPositiveButton(R.string.ok, new DialogInterface.OnClickListener() {
						...
						public void onClick(DialogInterface dialog, int which) {
								...
								if (pass1.getText().toString().compareTo(pass2.getText().toString()) == 0) {
									...
									if (!status.equals(LinphoneAccountCreator.Status.Ok)) {
										LinphoneUtils.displayErrorAlert(LinphoneUtils.errorForStatus(status)
												, MainActivity_with_Drawer.instance());
									} else {
										progress = ProgressDialog.show(MainActivity_with_Drawer.instance(), null, null);
										Drawable d = new ColorDrawable(ContextCompat.getColor(MainActivity_with_Drawer.instance(), R.color.colorE));
										d.setAlpha(200);
							...
							LinphoneUtils.displayErrorAlert(LinphoneUtils.errorForStatus(status), MainActivity_with_Drawer.instance());
						}
					});
    	...
    	delete.setOnPreferenceClickListener(new OnPreferenceClickListener() {
	        public boolean onPreferenceClick(Preference preference) {
				...
				// MainActivity_with_Drawer.instance().refreshAccounts();
	...
	public void onResume() {
		...
		if (MainActivity_with_Drawer.isInstanciated()) {
			// MainActivity_with_Drawer.instance().selectMenu(FragmentsAvailable.SETTINGS);
		}
	}
	...
	public void onPause() {
		...
		if (MainActivity_with_Drawer.isInstanciated()) {
			...
			MainActivity_with_Drawer.instance().isNewProxyConfig();
			LinphoneManager.getLc().refreshRegisters();
			// MainActivity_with_Drawer.instance().hideTopBar();
		}
	}
	...
	public void onAccountCreatorPasswordUpdated(LinphoneAccountCreator accountCreator, LinphoneAccountCreator.Status status) {
		...
		if (status.equals(LinphoneAccountCreator.Status.Ok)) {
			...
			LinphoneUtils.displayErrorAlert(getString(R.string.pref_password_changed), MainActivity_with_Drawer.instance());
		} else {
			LinphoneUtils.displayErrorAlert(LinphoneUtils.errorForStatus(status), MainActivity_with_Drawer.instance());
		}
	}
