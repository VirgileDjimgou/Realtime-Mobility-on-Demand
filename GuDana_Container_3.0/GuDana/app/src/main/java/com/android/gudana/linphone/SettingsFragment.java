...
import com.android.gudana.linphone.ui.LedPreference;
...
public class SettingsFragment extends PreferencesListFragment {
	...
	public void onCreate(Bundle bundle) {
		...
		removePreviousPreferencesFile(); // Required when updating the lin_preferences order
		...
		mListener = new LinphoneCoreListenerBase() {
			...
			public void ecCalibrationStatus(LinphoneCore lc, final EcCalibratorStatus status, final int delayMs, Object data) {
				...
				if (status == EcCalibratorStatus.DoneNoEcho) {
					...
					Log.i("Set lin_audio mode on 'Normal'");
	...
	private void initSettings() {
		...
		findPreference(getString(R.string.pref_add_account_key)).setOnPreferenceClickListener(new OnPreferenceClickListener() {
			...
			public boolean onPreferenceClick(Preference preference) {
				...
				MainActivity_with_Drawer.instance().displayAccountSettings(nbAccounts);
				return true;
			}
		});
	}
	...
	private void initAccounts() {
		...
		for (int i = 0; i < nbAccounts; i++) {
			...
			{
				public boolean onPreferenceClick(Preference preference) {
					MainActivity_with_Drawer.instance().displayAccountSettings(accountId);
					return false;
				}
			});
			...
			public boolean onPreferenceClick(Preference preference) {
				synchronized (SettingsFragment.this) {
					...
					if (recordAudio == PackageManager.PERMISSION_GRANTED) {
						...
						MainActivity_with_Drawer.instance().checkAndRequestRecordAudioPermissionForEchoCanceller();
					}
				}
				return true;
			}
			...
			public boolean onPreferenceClick(Preference preference) {
				synchronized (SettingsFragment.this) {
					...
					if (recordAudio == PackageManager.PERMISSION_GRANTED) {
						...
						MainActivity_with_Drawer.instance().checkAndRequestRecordAudioPermissionsForEchoTester();
					}
				}
				return true;
			}
	...
	private void setVideoPreferencesListener() {
		...
		findPreference(getString(R.string.pref_overlay_key)).setOnPreferenceChangeListener(new OnPreferenceChangeListener() {
			...
			public boolean onPreferenceChange(Preference preference, Object newValue) {
				boolean enable = (Boolean) newValue;
				if (enable) {
					if (MainActivity_with_Drawer.instance().checkAndRequestOverlayPermission()) {
			...
			public boolean onPreferenceChange(Preference preference, Object newValue) {
				...
				if (use) {
					...
					if (readExternalStorage == PackageManager.PERMISSION_GRANTED) {
						...
						MainActivity_with_Drawer.instance().checkAndRequestReadExternalStoragePermissionForDeviceRingtone();
					}
				} else {
