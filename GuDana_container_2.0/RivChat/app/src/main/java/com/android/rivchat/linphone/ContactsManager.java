	...
	private class ContactsFetchTask extends AsyncTask<Void, List<LinphoneContact>, List<LinphoneContact>> {
		...
		protected List<LinphoneContact> doInBackground(Void... params) {
			...
			if (hasContactsAccess()) {
				...
				Log.w("[Permission] Read lin_contacts permission wasn't granted, only fetch LinphoneFriends");
			}
			...
			for (LinphoneContact contact : contacts) {
				// This will only get name & picture informations to be able to quickly display lin_contacts list
				contact.minimalRefresh();
			}
			Collections.sort(contacts);

			// Public the current list of lin_contacts without all the informations populated
